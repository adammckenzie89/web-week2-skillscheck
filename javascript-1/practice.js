...
// First, with an arrow function let's use .map to create a new array that doubles each number. Call the new array 'doubled'
 ...
 let doubled = myNumbers.map(function(myNumbers){
	 return myNumbers * 2;
 })
...
let filtered = myNumbers.filter(function(myNumbers){
	return myNumbers > 100;
})
...
let total = myNumbers.reduce(function(acc, myNumbers){
	return acc += myNumbers;
})
...
let forTheLoveOfGeorge = notGeorge.map(function(i, notGeorge){
	 return notGeorge = notGeorge[i] = 'George';
})
...
let enemies = people.filter(function(ele){
	if(ele.friend === false){
		return ele;
	}
})
...
let totallyAwesome = people.reduce(function(total, ele){
	return total += ele.awesomeLevel;
},0)
